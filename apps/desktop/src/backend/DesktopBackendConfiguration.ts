...
interface PrimaryBootstrapInput extends SharedBootstrapInput {
  readonly apnsCredentials:
    | import("@t3tools/contracts").DesktopApnsBootstrapCredentials
    | undefined;
}
...
const resolvePrimaryStartConfig = Effect.fn("desktop.backendConfiguration.resolvePrimary")(
  ...
  > {
    ...
    const bootstrap = {
      ...
      ...(input.apnsCredentials ? { apnsCredentials: input.apnsCredentials } : {}),
    };
...
export const makeWithApnsCredentials = (
  loadApnsCredentials: Effect.Effect<
    Option.Option<import("@t3tools/contracts").DesktopApnsBootstrapCredentials>,
    DesktopApnsCredentialsStore.DesktopApnsCredentialsError
  >,
) =>
  Effect.gen(function* () {
    const environment = yield* DesktopEnvironment.DesktopEnvironment;
      ...
      const apnsCredentials =
        environment.platform === "darwin"
          ? Option.getOrUndefined(
              yield* loadApnsCredentials.pipe(
                Effect.catch((error) =>
                  Effect.logWarning("APNs credentials could not be loaded for backend bootstrap", {
                    error: error.message,
          ...
          : undefined;
      return yield* resolvePrimaryStartConfig({ ...shared, apnsCredentials }).pipe(
        Effect.provideService(DesktopEnvironment.DesktopEnvironment, environment),
...
export const make = makeWithApnsCredentials(Effect.succeed(Option.none()));

export const makeWithStoredApnsCredentials = Effect.gen(function* () {
  const store = yield* DesktopApnsCredentialsStore.DesktopApnsCredentialsStore;
  return yield* makeWithApnsCredentials(store.loadForBootstrap);
});

export const layer = Layer.effect(DesktopBackendConfiguration, make);
export const layerWithStoredApnsCredentials = Layer.effect(
  DesktopBackendConfiguration,
  makeWithStoredApnsCredentials,
);
