...
function imageSourceFromRecord(record: Record<string, unknown>): string | null {
  ...
  if (type === "inputImage") return asNonEmptyString(record.imageUrl);
  if (type === "input_image") return asNonEmptyString(record.image_url);
  if (type === "imageView") return asNonEmptyString(record.path);
  if (type === "imageGeneration") return imageGenerationSource(record);
  return null;
}
